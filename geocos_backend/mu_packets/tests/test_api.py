...
from geocos_backend.iotdb.exceptions import IoTDBWriteError
from geocos_backend.users.tests.factories import UserFactory
...
def test_create_packet_success(api_client: APIClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("geocos_backend.mu_packets.api.views.ingest_packet", fake_ingest)
    ...
    assert response.data == {"device": "root.geocos.factory.unit1", "records_written": 1}
...
def test_create_packet_iotdb_write_failure(api_client: APIClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("geocos_backend.mu_packets.api.views.ingest_packet", failing_ingest)
